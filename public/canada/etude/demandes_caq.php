...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $pdo) {
    ...
    if (empty($erreurs)) {
        $documents_requis = [
            ...
            'lettre_acceptation' => 'Lettre d\'acceptation du Québec'
        ];
        ...
        if (empty($erreurs)) {
            try {
                $sql = "INSERT INTO demandes_caq 
                        (user_id, nom_complet, email, telephone, date_naissance, pays_origine,
                         adresse, ville, code_postal, pays_residence, passeport_path, 
                         preuve_fonds_path, lettre_acceptation_path, statut, date_soumission) 
                        VALUES 
                        (:user_id, :nom_complet, :email, :telephone, :date_naissance, :pays_origine,
                         :adresse, :ville, :code_postal, :pays_residence, :passeport_path,
                         :preuve_fonds_path, :lettre_acceptation_path, 'nouveau', NOW())";
                ...
                $stmt->execute([
                    ...
                    ':lettre_acceptation_path' => $documents_uploades['lettre_acceptation']
                ]);
...
<head>
    ...
    <title>CAQ Québec - Demande de Certificat d'Acceptation</title>
...
<body>
    <div class="container">
        <header class="card">
            <h1><i class="fas fa-graduation-cap"></i> Certificat d'Acceptation du Québec (CAQ)</h1>
            <p>Formulaire de demande pour études au Québec</p>
        </header>
        ...
        <?php if ($succes): ?>
            <div class="alert alert-success">
                <h3><i class="fas fa-check-circle"></i> Demande CAQ enregistrée !</h3>
                <p><strong>Référence :</strong> CAQ<?php echo str_pad($id_demande, 6, '0', STR_PAD_LEFT); ?></p>
                <p>Votre demande de CAQ a été soumise avec succès.</p>
                <p><a href="mes_demandes_caq.php" style="color: #155724; text-decoration: underline;">Voir mes demandes CAQ</a></p>
            </div>
        <?php endif; ?>
        ...
        <form method="POST" action="" enctype="multipart/form-data" class="card" id="demandeForm">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="pays_origine" name="pays_origine" required 
                               value="<?php echo htmlspecialchars($_POST['pays_origine'] ?? ''); ?>">
                    </div>
                    
                    <div class="form-group">
                        <label for="pays_residence">Pays de résidence actuelle</label>
                        <input type="text" id="pays_residence" name="pays_residence" 
                               value="<?php echo htmlspecialchars($_POST['pays_residence'] ?? ''); ?>">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="adresse" name="adresse" rows="3" placeholder="Votre adresse complète actuelle..."><?php echo htmlspecialchars($_POST['adresse'] ?? ''); ?></textarea>
                </div>
            </div>

            <!-- Documents -->
            <div class="form-section">
                <h2 class="section-title"><i class="fas fa-file-upload"></i> Documents requis pour le CAQ</h2>
                ...
                <div class="alert alert-warning">
                    ...
                    <strong>Documents obligatoires :</strong> Passeport et Lettre d'acceptation<br>
                    <strong>Documents optionnels :</strong> Preuve de fonds
                </div>
                ...
                <div class="documents-grid">
                    ...
                    <div class="document-card">
                        <label class="required">Lettre d'acceptation du Québec <span class="badge badge-required">OBLIGATOIRE</span></label>
                        <div class="file-upload-container">
                            <div class="file-input-wrapper">
                                ...
                                <input type="file" id="lettre_acceptation" name="lettre_acceptation[]" class="file-input" required 
                                       accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" multiple>
                            </div>
            ...
            <button type="submit" class="btn-submit">
                <i class="fas fa-paper-plane"></i> Soumettre la demande CAQ
            </button>
        </form>
        <?php endif; ?>
    </div>
    ...
    <script>
        ...
        const dateDebutInput = document.getElementById('date_naissance');
        if (dateDebutInput) {
            const today = new Date();
            const maxDate = new Date(today.getFullYear() - 16, today.getMonth(), today.getDate());
            dateDebutInput.max = maxDate.toISOString().split('T')[0];
        }
    </script>
</body>
</html>