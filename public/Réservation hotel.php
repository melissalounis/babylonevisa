...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $pays_arrivee = trim($_POST['pays_arrivee']);
    $ville_arrivee = trim($_POST['ville_arrivee']);
    $date_depart = $_POST['date_depart'];
    $date_retour = $_POST['date_retour'];
    $classe = $_POST['classe'];
    $compagnie_preferee = trim($_POST['compagnie_preferee']);
    ...
    $passagers = [];
    for ($i = 1; $i <= $nombre_adultes + $nombre_enfants; $i++) {
        $civilite = $_POST["civilite_$i"];
        $nom = trim($_POST["nom_$i"]);
        $prenom = trim($_POST["prenom_$i"]);
        $date_naissance = $_POST["date_naissance_$i"];
        $numero_passeport = trim($_POST["numero_passeport_$i"]);
        $expiration_passeport = $_POST["expiration_passeport_$i"];
        $nationalite = trim($_POST["nationalite_$i"]);
    ...
    foreach ($passagers as $index => $passager) {
        ...
        if (empty($passager['nom'])) $erreurs[] = "Le nom du passager $numero_passager est requis";
        if (empty($passager['prenom'])) $erreurs[] = "Le prénom du passager $numero_passager est requis";
        if (empty($passager['date_naissance'])) $erreurs[] = "La date de naissance du passager $numero_passager est requise";
        if (empty($passager['numero_passeport'])) $erreurs[] = "Le numéro de passeport du passager $numero_passager est requis";
        if (empty($passager['expiration_passeport'])) $erreurs[] = "La date d'expiration du passeport du passager $numero_passager est requise";
        if (empty($passager['nationalite'])) $erreurs[] = "La nationalité du passager $numero_passager est requise";
    }
    ...
    if (empty($erreurs)) {
        ...
        // Créer le nom complet (premier passager)
        $nom_complet = $passagers[0]['prenom'] . ' ' . $passagers[0]['nom'];
        $nationalite = $passagers[0]['nationalite'];
        
        // Calcul du prix estimé
        $prix_base_adulte = 300;
        $prix_base_enfant = 200;
        $prix_estime = ($nombre_adultes * $prix_base_adulte) + ($nombre_enfants * $prix_base_enfant);
        if ($type_vol == 'aller_retour') $prix_estime *= 1.8;
        if ($classe == 'affaires') $prix_estime *= 2;
        if ($classe == 'premiere') $prix_estime *= 3;
        ...
        try {
            ...
            $sql_demande = "INSERT INTO demandes_billets_avoin 
                           (user_email, numero_dossier, nom_complet, email, telephone, nationalite, type_vol, 
                            pays_depart, ville_depart, pays_arrivee, ville_arrivee, date_depart, 
                            date_retour, classe, compagnie_preferee, nombre_adultes, nombre_enfants, 
                            ages_enfants, baggage_main, baggage_soute, commentaires, prix_estime, statut) 
                           VALUES 
                           (:user_email, :numero_dossier, :nom_complet, :email, :telephone, :nationalite, :type_vol,
                            :pays_depart, :ville_depart, :pays_arrivee, :ville_arrivee, :date_depart,
                            :date_retour, :classe, :compagnie_preferee, :nombre_adultes, :nombre_enfants,
                            :ages_enfants, :baggage_main, :baggage_soute, :commentaires, :prix_estime, 'en_attente')";
            ...
            $message_success = "
                <div style='text-align: center; padding: 20px; background: #d4edda; color: #155724; border-radius: 8px; margin-bottom: 20px; border: 1px solid #c3e6cb;'>
                    ...
                    <div style='background: #f8f9fa; padding: 15px; border-radius: 8px; margin-top: 15px; text-align: left;'>
                        ...
                        <p>👥 Passagers: $nombre_adultes adulte(s) + $nombre_enfants enfant(s)</p>
                        <p>💼 Classe: " . ucfirst($classe) . "</p>
                        <p>💰 Prix estimé: " . number_format($prix_estime, 2, ',', ' ') . " €</p>
                    </div>
                    <div style='margin-top: 20px;'>
                        <a href='mes_demandes.php' class='btn' style='display: inline-block; background: #28a745; color: white; padding: 10px 20px; border-radius: 5px; text-decoration: none;'>
...
<head>
    ...
    <title>Réservation de Billets d'Avion</title>
...
<body>
            ...
            <div class="card-body">
                ...
                <div class="user-info">
                    ...
                    <span style="float: right;">
                        <a href="mes_demandes.php" style="color: var(--accent); text-decoration: none;">
                ...
                <form method="POST" action="" id="reservationForm">
                    ...
                    <h3 class="section-title"><i class="fas fa-users"></i> Informations des Passagers</h3>
                    
                    <div class="form-grid">
                        <div class="form-group">
                            <label for="nombre_adultes">Nombre d'adultes*</label>
                            <select id="nombre_adultes" name="nombre_adultes" class="form-control" required onchange="updatePassengers()">
                                <?php for ($i = 1; $i <= 9; $i++): ?>
                        ...
                        <div class="form-group">
                            ...
                            <select id="nombre_enfants" name="nombre_enfants" class="form-control" onchange="updatePassengers()">
                                <?php for ($i = 0; $i <= 6; $i++): ?>
    ...
    <script>
        ...
        function updatePassengers() {
            ...
            const totalPassagers = nombreAdultes + nombreEnfants;
            const container = document.getElementById('passengers-container');
            container.innerHTML = '';
            
            for (let i = 1; i <= totalPassagers; i++) {
                const isEnfant = i > nombreAdultes;
                const typePassager = isEnfant ? 'enfant' : 'adulte';
                const passengerHtml = `
                    <div class="passenger-section">
                        <div class="passenger-header">
                            <div class="passenger-title">Passager ${i} (${typePassager})</div>
                        </div>
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <select name="civilite_${i}" class="form-control" required>
                                    ...
                                    ${isEnfant ? '<option value="Enfant">Enfant</option>' : ''}
                                </select>
                            </div>
                            <div class="form-group">
                                <label>Nom*</label>
