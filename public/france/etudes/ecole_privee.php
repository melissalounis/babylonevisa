...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $required_fields = [
            'pays_etudes', 'niveau_etudes', 'domaine_etudes', 
            'nom', 'prenom', 'date_naissance', 'lieu_naissance', 'nationalite',
            'type_piece_identite', 'num_piece_identite', 'date_delivrance_piece', 'date_expiration_piece',
            'adresse', 'telephone', 'email', 
            'situation_familiale', 'profession_candidat', 'categorie_socio_pro',
            'budget_etudes', 'session_formation',
            'a_garant_france'
        ];
        ...
        // 🔹 Vérification garant si "oui"
        if ($_POST['a_garant_france'] === 'oui') {
            $garant_fields = ['nom_garant', 'prenom_garant', 'adresse_garant', 'telephone_garant', 'email_garant', 'lien_parente_garant'];
            foreach ($garant_fields as $field) {
                if (empty($_POST[$field])) {
                    $errors[] = "Le champ $field est obligatoire lorsque vous avez un garant en France.";
                }
            }
        }

        // 🔹 Vérification du certificat de scolarité (obligatoire sauf pour master terminé)
        $niveau_etudes = $_POST['niveau_etudes'] ?? '';
        if ($niveau_etudes !== 'master_termine' && empty($_FILES['certificat_scolarite']['name'])) {
            $errors[] = "Le certificat de scolarité est obligatoire pour tous les niveaux sauf Master terminé.";
        }

        // 🔹 Vérification des pièces obligatoires
        $required_files = ['copie_piece_identite', 'photo_identite'];
        ...
        if (!empty($errors)) {
            ...
            $date_delivrance_piece = $_POST['date_delivrance_piece'] ?? '';
            $date_expiration_piece = $_POST['date_expiration_piece'] ?? '';
            
            // 🔹 Situation familiale
            $situation_familiale = $_POST['situation_familiale'] ?? '';
            $profession_candidat = $_POST['profession_candidat'] ?? '';
            $categorie_socio_pro = $_POST['categorie_socio_pro'] ?? '';
            
            // 🔹 Budget et session (remplace les professeurs)
            $budget_etudes = $_POST['budget_etudes'] ?? '';
            $session_formation = $_POST['session_formation'] ?? '';
            
            // 🔹 Garant en France
            $a_garant_france = $_POST['a_garant_france'] ?? 'non';
            $nom_garant = $_POST['nom_garant'] ?? '';
            $prenom_garant = $_POST['prenom_garant'] ?? '';
            $adresse_garant = $_POST['adresse_garant'] ?? '';
            $telephone_garant = $_POST['telephone_garant'] ?? '';
            $email_garant = $_POST['email_garant'] ?? '';
            $lien_parente_garant = $_POST['lien_parente_garant'] ?? '';
            ...
            // 🔹 Insertion dans `demandes_ecoles_privees`
            $stmt = $pdo->prepare("INSERT INTO demandes_ecoles_privees 
                ...
                situation_familiale, profession_candidat, categorie_socio_pro,
                budget_etudes, session_formation,
                a_garant_france, nom_garant, prenom_garant, adresse_garant, telephone_garant, email_garant, lien_parente_garant,
                niveau_francais, tests_francais, score_test, test_anglais, score_anglais, boite_pastel,
                email_pastel, mdp_pastel, releves_annees, autres_documents, statut) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'en_attente')");
            ...
            function saveFile($file, $type, $demande_id, $pdo, $uploadDir) {
                if (!empty($file['name']) && $file['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $filepath)) {
                        $stmt = $pdo->prepare("INSERT INTO demandes_ecoles_privees_fichiers 
            ...
            if (isset($_FILES['copie_piece_identite'])) {
                saveFile($_FILES['copie_piece_identite'], 'copie_piece_identite', $demande_id, $pdo, $uploadDir);
            }
            if (isset($_FILES['photo_identite'])) {
                saveFile($_FILES['photo_identite'], 'photo_identite', $demande_id, $pdo, $uploadDir);
            }

            // 🔹 Traitement du certificat de scolarité (obligatoire sauf pour master terminé)
            if ($niveau_etudes !== 'master_termine' && isset($_FILES['certificat_scolarite']) && !empty($_FILES['certificat_scolarite']['name'])) {
            ...
            // 🔹 Traitement pièce d'identité du garant
            if ($a_garant_france === 'oui' && isset($_FILES['piece_identite_garant']) && !empty($_FILES['piece_identite_garant']['name'])) {
                saveFile($_FILES['piece_identite_garant'], 'piece_identite_garant', $demande_id, $pdo, $uploadDir);
            }

            // 🔹 Redirection confirmation
            header("Location: confirmation_ecoles_privees.php?id=" . $demande_id);
            exit;
        }
    }
...
<head>
  ...
  <title>Formulaire Écoles Privées</title>
...
<div class="container">
  <header>
    <h1><i class="fas fa-graduation-cap"></i> Formulaire Écoles Privées</h1>
    <p>Demande d'inscription pour les écoles privées en France</p>
  </header>
  ...
  <div class="form-content">
    <form method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
      ...
      <div class="form-section">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="text" name="num_piece_identite" required>
          </div>
          <div class="form-group">
            <label class="required">Date de délivrance</label>
            <input type="date" name="date_delivrance_piece" required>
          </div>
          <div class="form-group">
            <label class="required">Date d'expiration</label>
            <input type="date" name="date_expiration_piece" required>
          </div>
        </div>
      ...
      <div class="form-section">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input type="text" name="profession_candidat" required>
          </div>
          <div class="form-group">
            ...
            <select name="categorie_socio_pro" required>
              ...
              <option value="cadre">Cadre, profession intellectuelle</option>
      ...
      <!-- Budget et session -->
      <div class="form-section">
        <h3><i class="fas fa-euro-sign"></i> Budget et session de formation</h3>
        
        <div class="form-row">
          <div class="form-group">
            <label class="required">Budget annuel d'études (€)</label>
            <select name="budget_etudes" required>
          ...
          <div class="form-group">
            <label class="required">Session de formation</label>
            <select name="session_formation" required>
              <option value="">-- Sélectionnez la session --</option>
              <option value="session_septembre">Session de Septembre (Rentrée classique)</option>
              <option value="session_janvier">Session de Janvier (Rentrée décalée)</option>
              <option value="session_avril">Session d'Avril (Rentrée de printemps)</option>
            </select>
          </div>
        </div>
        ...
        <div class="form-group">
          <div class="conditional-section">
            <h4><i class="fas fa-info-circle"></i> Informations sur les sessions</h4>
            <p><strong>Session de Septembre :</strong> Rentrée classique, début en septembre, inscriptions jusqu'en juillet</p>
            <p><strong>Session de Janvier :</strong> Rentrée décalée, début en janvier, inscriptions jusqu'en décembre</p>
            <p><strong>Session d'Avril :</strong> Rentrée de printemps, début en avril, inscriptions jusqu'en mars</p>
          </div>
        </div>
      </div>
        ...
        <div id="garant_section" class="conditional-section hidden">
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input type="text" name="prenom_garant">
            </div>
          </div>
          
          <div class="form-group">
            <label class="required">Adresse complète du garant</label>
            <textarea name="adresse_garant" rows="3"></textarea>
          </div>
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input type="text" name="lien_parente_garant" placeholder="Ex: Oncle, Tante, Ami...">
            </div>
          </div>
      ...
      <div class="form-section">
        ...
        <div class="form-group">
          <label class="required">
            <input type="checkbox" name="ecoles_privees" required>
            Je m'engage à suivre la procédure d'inscription aux écoles privées
          </label>
        </div>
      </div>
      ...
      <div class="form-section" style="text-align: center;">
        <button type="submit" class="btn-submit"><i class="fas fa-paper-plane"></i> Soumettre la demande Écoles Privées</button>
      </div>
    </form>
  </div>
  
  <footer>
    <p>© 2025 Écoles Privées en France. Tous droits réservés.</p>
  </footer>
</div>
...
function toggleGarantSection() {
    ...
    garantSection.style.display = (hasGarant === "oui") ? "block" : "none";
    
    // Rendre les champs obligatoires ou non
    const garantFields = garantSection.querySelectorAll("input, textarea, select");
    garantFields.forEach(field => {
        field.required = (hasGarant === "oui");
    });
}
