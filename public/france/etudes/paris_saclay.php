...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $required_fields = [
            ...
            'nom_prof1', 'email_prof1', 'nom_prof2', 'email_prof2',
            'a_garant_france'
        ];
        ...
        if (!empty($errors)) {
            ...
            // 🔹 Informations professeurs (remplace les parents)
            $nom_prof1 = $_POST['nom_prof1'] ?? '';
            $email_prof1 = $_POST['email_prof1'] ?? '';
            $nom_prof2 = $_POST['nom_prof2'] ?? '';
            $email_prof2 = $_POST['email_prof2'] ?? '';
            ...
            // 🔹 Insertion dans `demandes_paris_saclay`
            $stmt = $pdo->prepare("INSERT INTO demandes_paris_saclay 
            ...
            function saveFile($file, $type, $demande_id, $pdo, $uploadDir) {
                if (!empty($file['name']) && $file['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $filepath)) {
                        $stmt = $pdo->prepare("INSERT INTO demandes_paris_saclay_fichiers 
            ...
            header("Location: confirmation_paris_saclay.php?id=" . $demande_id);
            exit;
        }
    }
...
<head>
  ...
  <title>Formulaire Paris-Saclay</title>
...
<div class="container">
  <header>
    <h1><i class="fas fa-graduation-cap"></i> Formulaire Paris-Saclay</h1>
    <p>Demande d'inscription pour l'Université Paris-Saclay</p>
  </header>
  ...
  <div class="form-content">
    <form method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
      ...
      <div class="form-section">
        <h3><i class="fas fa-chalkboard-teacher"></i> Références académiques</h3>
        
        <h4>Professeur référent 1</h4>
        <div class="form-row">
          <div class="form-group">
            <label class="required">Nom du professeur</label>
            <input type="text" name="nom_prof1" required placeholder="Nom complet du professeur">
          </div>
          <div class="form-group">
            <label class="required">Email du professeur</label>
            <input type="email" name="email_prof1" required placeholder="email@universite.fr">
          </div>
        </div>
        
        <h4>Professeur référent 2</h4>
        <div class="form-row">
          <div class="form-group">
            <label class="required">Nom du professeur</label>
            <input type="text" name="nom_prof2" required placeholder="Nom complet du professeur">
          </div>
          <div class="form-group">
            <label class="required">Email du professeur</label>
            <input type="email" name="email_prof2" required placeholder="email@universite.fr">
          </div>
        </div>
      </div>
      ...
      <div class="form-section">
        ...
        <div class="form-group">
          <label class="required">
            <input type="checkbox" name="paris_saclay" required>
            Je m'engage à suivre la procédure d'inscription à l'Université Paris-Saclay
          </label>
        </div>
      </div>
      ...
      <div class="form-section" style="text-align: center;">
        <button type="submit" class="btn-submit"><i class="fas fa-paper-plane"></i> Soumettre la demande Paris-Saclay</button>
      </div>
    </form>
  </div>
  
  <footer>
    <p>© 2025 Université Paris-Saclay. Tous droits réservés.</p>
  </footer>
</div>
