...
try {
    ...
    $checkColumns = $pdo->prepare("
        SHOW COLUMNS FROM demandes_turquie 
        WHERE Field IN ('passeport', 'diplomes', 'releves_notes', 'photo_identite', 'certificat_langue')
    ");
    $checkColumns->execute();
    $existingColumns = $checkColumns->fetchAll(PDO::FETCH_COLUMN);
    
    $colonnesDocuments = ['passeport', 'diplomes', 'releves_notes', 'photo_identite', 'certificat_langue'];
    $colonnesManquantes = array_diff($colonnesDocuments, $existingColumns);
    
    // Si des colonnes manquent, les créer
    if (!empty($colonnesManquantes)) {
        foreach ($colonnesManquantes as $colonne) {
            try {
                $pdo->exec("ALTER TABLE demandes_turquie ADD COLUMN $colonne VARCHAR(255) NULL");
            } catch (PDOException $e) {
                $erreurs[] = "Erreur création colonne $colonne: " . $e->getMessage();
            }
        }
    }
    ...
    $stmt = $pdo->prepare("
        SELECT id, specialite, statut, certificat_type
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$demande_valide) {
            ...
            foreach ($colonnesDocuments as $doc_type) {
                if (isset($_FILES[$doc_type]) && $_FILES[$doc_type]['error'] === UPLOAD_ERR_OK) {
                    $file = $_FILES[$doc_type];
                    
                    // Vérification de la taille
                    if ($file['size'] > $max_size) {
                        $erreurs[] = "Le fichier " . formatDocumentName($doc_type) . " est trop volumineux (max 5MB)";
                        continue;
                    }
                    ...
                    if (!in_array($file_extension, $allowed_types)) {
                        $erreurs[] = "Type de fichier non autorisé pour " . formatDocumentName($doc_type) . " (PDF, JPG, PNG, DOC uniquement)";
                        continue;
                    }
                    ...
                    if (move_uploaded_file($file['tmp_name'], $file_path)) {
                        ...
                        $erreurs[] = "Erreur lors de l'upload du document " . formatDocumentName($doc_type);
                    }
                }
            }
            ...
            if (!empty($documents_uploades) && empty($erreurs)) {
                try {
                    ...
                    $checkUpdated = $pdo->prepare("SHOW COLUMNS FROM demandes_turquie LIKE 'updated_at'");
                    $checkUpdated->execute();
                    if ($checkUpdated->fetch()) {
                    ...
                    if ($stmt_update->execute($params)) {
                        ...
                        header("Location: documents_turquie.php");
                        exit;
                    } else {
                        $pdo->rollBack();
    ...
    foreach ($demandes as &$demande) {
        try {
            ...
            // Ajouter certificat de langue si requis
            ...
            foreach ($documents_requis as $doc) {
                if (empty($demande['documents'][$doc])) {
            ...
            $demande['documents'] = [];
            $demande['documents_complets'] = false;
            $demande['documents_manquants'] = $documents_requis;
        }
    }
...
<body>
                                    ...
                                    <div class="upload-form">
                                        <form method="POST" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group">
                                                <label>Ajouter des documents manquants :</label>
                                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                                    <?php foreach ($documents_requis as $doc_type): ?>
                                                        <?php if (empty($demande['documents'][$doc_type])): ?>
                                                            <div class="file-input-group">
                                                                <label for="<?php echo $doc_type . '_' . $demande['id']; ?>">
                                                                    <?php echo formatDocumentName($doc_type); ?>
                                                                </label>
                                                                <input type="file" 
                                                                       id="<?php echo $doc_type . '_' . $demande['id']; ?>"
                                            ...
                                            <?php if (count($demande['documents_manquants']) > 0): ?>
                                                <button type="submit" class="btn btn-primary">
                                                    <i class="fas fa-upload"></i> Uploader les documents manquants
                                                </button>
                                            <?php else: ?>
                                                <div class="alert alert-success">
                                                    <i class="fas fa-check-circle"></i> Tous les documents sont présents
                                                </div>
                                            <?php endif; ?>
                                        </form>
                                    </div>
    ...
    <script>
        ...
        console.log('Page documents_turquie.php chargée avec succès');
    </script>
</body>
</html>