...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && isset($_POST['id'])) {
            ...
            switch($action) {
                case 'changer_statut':
                    if (isset($_POST['statut'])) {
                        $statut = $_POST['statut'];
                        $stmt = $pdo->prepare("UPDATE demandes_luxembourg SET statut = ?, date_modification = NOW() WHERE id = ?");
                ...
                case 'supprimer':
                    ...
                    $stmt = $pdo->prepare("SELECT chemin_fichier FROM demandes_luxembourg_fichiers WHERE demande_id = ?");
                    $stmt->execute([$id]);
                    $fichiers = $stmt->fetchAll(PDO::FETCH_COLUMN);
                    
                    // Supprimer les fichiers physiquement
                    foreach ($fichiers as $fichier) {
                        $filepath = __DIR__ . "/../../../uploads/luxembourg/" . $fichier;
                    ...
                    $stmt = $pdo->prepare("DELETE FROM demandes_luxembourg_fichiers WHERE demande_id = ?");
                    $stmt->execute([$id]);
                    
                    // Supprimer la demande
                    $stmt = $pdo->prepare("DELETE FROM demandes_luxembourg WHERE id = ?");
                    $stmt->execute([$id]);
        ...
        header('Location: admin_etude_luxembourg.php');
        exit();
    }
}
...
try {
    ...
    if (!empty($search)) {
        $where_conditions[] = "(nom LIKE ? OR email LIKE ? OR telephone LIKE ? OR nationalite LIKE ?)";
        $search_term = "%$search%";
        $params[] = $search_term;
        $params[] = $search_term;
        $params[] = $search_term;
    ...
    $count_sql = "SELECT COUNT(*) FROM demandes_luxembourg $where_sql";
    ...
    $sql = "SELECT * FROM demandes_luxembourg $where_sql ORDER BY date_creation DESC LIMIT $limit OFFSET $offset";
    ...
    $stats_sql = "
        SELECT 
            ...
            SUM(niveau = 'bachelor') as bachelor,
            SUM(niveau = 'master') as master,
            SUM(niveau = 'doctorat') as doctorat
    ...
    // Compter les tests de langue
    $langues_sql = "
        SELECT 
            COUNT(DISTINCT demande_id) as francais
        FROM demandes_luxembourg_fichiers 
        WHERE type_fichier = 'test_francais'
    ";
    $langues_result = $pdo->query($langues_sql);
    if ($langues_result) {
        $langues_stats = $langues_result->fetch(PDO::FETCH_ASSOC);
        $stats['francais'] = $langues_stats['francais'] ?? 0;
    }
...
<head>
    ...
    <title>Administration Études Luxembourg - Babylone Service</title>
    ...
    <style>
        ...
        .luxembourg-flag {
            background: linear-gradient(180deg, #ED2939 0%, #ED2939 33%, #FFFFFF 33%, #FFFFFF 66%, #00A2E0 66%, #00A2E0 100%);
            height: 4px;
            margin-bottom: 10px;
            border-radius: 2px;
        }
...
<body>
    <div class="wrapper">
        ...
        <nav class="sidebar">
            ...
            <ul class="nav flex-column p-3">
                ...
                <li class="nav-item mb-2">
                    <a href="admin_etude_luxembourg.php" class="nav-link text-white bg-dark rounded">
                        <i class="fas fa-graduation-cap me-2"></i>
                        Études Luxembourg
                    </a>
                </li>
        ...
        <div class="main-content">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <div class="luxembourg-flag"></div>
                    <h1 class="h3 mb-1">
                        <i class="fas fa-euro-sign text-primary me-2"></i>
                        Gestion des Études au Luxembourg
                    </h1>
                    <p class="text-muted mb-0">Consultez et gérez toutes les demandes d'études au Luxembourg</p>
                </div>
                ...
                <form method="GET" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="niveau" class="form-select" onchange="this.form.submit()">
                            ...
                            <option value="bachelor" <?php echo $niveau_filter === 'bachelor' ? 'selected' : ''; ?>>Bachelor</option>
                    ...
                    <div class="col-md-3">
                        <a href="admin_etude_luxembourg.php" class="btn btn-outline-secondary w-100">
                            ...
                            <?php if (empty($demandes)): ?>
                                <tr>
                                    <td colspan="9" class="text-center py-5">
                                        ...
                                        <p class="text-muted">Aucune demande d'études au Luxembourg trouvée</p>
                                        <?php if ($statut_filter !== 'tous' || $niveau_filter !== 'tous' || !empty($search)): ?>
                                            <a href="admin_etude_luxembourg.php" class="btn btn-primary">
                            ...
                            <?php else: ?>
                                <?php foreach($demandes as $demande): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            // Vérifier les tests de langue
                                            $tests_langue = [];
                                            try {
                                                $stmt = $pdo->prepare("SELECT type_fichier FROM demandes_luxembourg_fichiers WHERE demande_id = ? AND type_fichier LIKE 'test_%'");
                                            ...
                                            ?>
                                            <div class="small">
                                                <?php if (!empty($tests_langue)): ?>
                                                    <?php foreach($tests_langue as $test): ?>
                                                        <span class="badge bg-success mb-1"><?php echo $test; ?></span>
                                                    <?php endforeach; ?>
                                                <?php else: ?>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">Détails de la demande d'études au Luxembourg</h5>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="admin_etude_luxembourg.php" id="form-delete-user">
    ...
    // Charger les détails via AJAX - ⚠️ LIGNE À MODIFIER ⚠️
    fetch(`details_luxembourg.php?id=${demandeId}`)
        .then(response => {
            if (!response.ok) {
                ...
                <div class="details-container">
                    ...
                    <div class="row mb-4">
                        <div class="col-12">
                            ...
                            <p class="text-muted mb-0">Référence: LU-${demande.id.toString().padStart(6, '0')}</p>
                        </div>
                    </div>
                            ...
                            <div class="card h-100">
                                ...
                                <div class="card-body">
                                    <table class="table table-sm table-borderless">
                                        ...
                                        <tr>
                                            <td><strong>Tests de langue:</strong></td>
                                            <td>
                                                <div class="small">
                                                    ${demande.tests_langue && demande.tests_langue.length > 0 ? 
                                                        demande.tests_langue.map(test => `<span class="badge bg-success mb-1">${test}</span>`).join(' ') : 
                                                        '<span class="badge bg-secondary">Aucun test</span>'
                                                    }
                                                </div>
                                            </td>
                                        </tr>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="alert alert-info">
                                        ...
                                        La gestion des documents sera implémentée dans la version finale.
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        function changerStatut(id, nouveauStatut) {
            if (confirm('Êtes-vous sûr de vouloir changer le statut de cette demande ?')) {
                ...
                form.method = 'POST';
                form.action = 'admin_etude_luxembourg.php';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Administration études Luxembourg chargée');
        });
    </script>
</body>
</html>